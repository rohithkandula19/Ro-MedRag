...
WHAT CHANGED:
  - query() now passes session_id to the pipeline for conversation memory
  - Session deletion clears agent memory
...
class ChatService:
    ...
    async def query(
        ...
        """Run RAG agent pipeline and persist message + citations."""
        ...
        # ── Run RAG Agent (CHANGED: now passes session_id for memory) ──
        pipeline = get_pipeline()
        rag_result: RAGResponse = await pipeline.query(
            question=question.strip(),
            document_ids=chat.document_ids or None,
            mode=mode,
            session_id=session_id,  # NEW: enables conversation memory
        )
    ...
    async def delete_session(
        ...
        # ── NEW: Clear agent memory for this session ───────────────────
        pipeline = get_pipeline()
        pipeline.clear_session_memory(session_id)
